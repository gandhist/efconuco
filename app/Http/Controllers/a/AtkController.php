...
use App\AtkDetails;
use App\AtkStoreqHeader;
use App\AtkTroHeader;
use App\AtkOutHeader;
use App\Employee;
...
class AtkController extends Controller
{
    ...
    {
        $data_gudang = DB::select("select id, nama, lantai from master_gudang where kategori = 'ATK' ");
        foreach ($data_gudang as $key) {
            ...
            CASE
                WHEN 1=1 THEN 
                (SELECT a.balance
            FROM atk_storeq_details  a
            INNER JOIN 
            (
            SELECT MAX(id) AS id
            FROM atk_storeq_details 
        ...
        , c.harga
        FROM master_stock c
        WHERE c.kategori = 'ATK'
        ");
        $data['emp'] = Employee::all();
        $data['dd'] = Employee::all();
        
        return view('atk.info', $data);
    }
    ...
    public function restock(Request $request)
    {
        ...
        if ($request->isMethod('GET')) {
            return redirect('atk/info')->with('success','Pilih terlebih dahulu item yang akan di Re-Stock');
        }

        // validasi work here
        $cek_status = DB::table('atk_storeq_details')
       ...
       if ($cek_status->count() > 0) {
        ...
        //   return $id_headers;
          $pending = AtkStoreqHeader::whereIn('id', $id_headers)->get(['trans_no','id']);
          foreach ($pending as $key) {
              $trans_no[] = $key->trans_no;
          }
          $trans_no = implode(", ",$trans_no);
          return redirect('atk/info')->with('success',"selesaikan terlebih dahulu transaksi atas nomor : $trans_no");
       }
        //end of validasi
        // running number
        $code = "/RES/ATK/";
        ...
        , c.harga
        FROM master_stock c
        WHERE c.kategori = 'ATK' and c.kode_barang in ('$selected_id')
        ");
        $data['emp'] = Employee::all();
        $data['dd'] = Employee::all();
        $data['dd_vendor'] = Employee::all(); 
        
        return view('atk.restock', $data);
    }

    public function edit_restock($id)
    {
        $data['header'] = AtkStoreqHeader::find($id);
        ...
        foreach ($data_gudang as $key) {
            ...
            $string[] = "
            CASE
                WHEN 1=1 THEN (SELECT concat(id,'_',balance) FROM atk_storeq_details WHERE id_header = $id
        ...
        $data['dd'] = Employee::all();
        $data['dd_vendor'] = Employee::all(); 
        return view('atk.edit_restock', $data);
    }
    ...
    public function bulk_add(Request $request)
    {
        ...
        $name = str_replace('/','_',$request->trans_no);
         if ($files = $request->file('pr_files')) {
            $destinationPath = 'uploads/stock_atk/invoice'; // upload path
        ...
        elseif ($request->status == "PAID") {
            ...
            if ($files = $request->file('bukti_bayars')) {
                $destinationPath = 'uploads/stock_atk/bukti_bayar'; // upload path
            ...
            foreach ($master_barang as $key) {
                foreach ($gudang as $store) {
                    ...
                    if ($request->has($prefix)) {
                        $bal = AtkDetails::where('id_barang',$key->kode_barang)->where('gudang_id',$store->id)->whereIn('status',['PAID','TRANSFERED','OUT'])->orderBy('id','desc')->first(['balance']);
                        if (is_null($bal) ) {
                            $bal = 0;
                        }
                        else {
                        ...
                        $details = new AtkDetails;
                        $details->id_header = $header->id;
                        $details->id_barang = $key->kode_barang;
                        $details->stock_in = $request->$prefix;
                        $details->balance = $request->$prefix; // + $bal; saldo akan menambah jika status di update
    ...
    public function bulk_update(Request $request, $id)
    {
        ...
        $gudang = MasterGudang::all();
        $master_barang = MasterStock::all();
        $atk_storeq_details = AtkDetails::where('id_header',$id)->get();
        ...
        // update ke header
        $header = AtkStoreqHeader::find($id);
        $header->trans_no = $request->trans_no;
        $header->date_create = $request->tanggal;
        ...
        // delete gambar
         if ($files = $request->file('pr_files')) {
            Storage::delete('uploads/stock_atk/invoice'.$header->pr_file);
            $destinationPath = 'uploads/stock_atk/invoice'; // upload path
            $profileImage = "PR_".str_replace('/','_',$request->trans_no) . "." . $files->getClientOriginalExtension();
            $files->move($destinationPath, $profileImage);
            $header->pr_file = $profileImage;
         }
        $header->vendor_id = $request->vendor;
        ...
        elseif ($request->status == "PAID") {
            $header->paid_date = $request->payment_date;
            if ($files = $request->file('bukti_bayars')) {
                Storage::delete('uploads/stock_atk/bukti_bayar'.$header->bukti_bayar);
                $destinationPath = 'uploads/stock_atk/bukti_bayar'; // upload path
                $profileImage = "BB_".str_replace('/','_',$request->trans_no) . "." . $files->getClientOriginalExtension();
                $files->move($destinationPath, $profileImage);
                $header->bukti_bayar = $profileImage;
             }
        }
        ...
        foreach ($master_barang as $key) {
            foreach ($gudang as $store) {
                foreach ($atk_storeq_details as $storeq_det) {
                    ...
                    if ($request->has($prefix)) {
                        ...
                        if ($request->status == "PAID") {
                            $details_delete = AtkDetails::find($storeq_det->id);
                        ...
                        // jika tidak paid 
                            ...
                            $details = AtkDetails::find($storeq_det->id);
                            $details->id_header = $header->id;
                            $details->id_barang = $key->kode_barang;
                            $details->tanggal = $request->tanggal;
                            $details->stock_in = $request->$prefix;
                ...
                } // end foreach atk_storeq_details
            } // end foreach gudang
        ...
        // end of save to details
            ...
            return response()->json([
                'status' => true,
                'message' => 'Transaksi berhasil di perbarui'
            ],200);
        }
    ...
    public function purchase()
    {
        ...
        return view('atk.purchasing.index', $data);
    }
    ...
    public function pr_list(Request $request)
    {
        ...
        $totalData = AtkStoreqHeader::count();
        ...
        if (empty($request->input('search.value'))) {
            ...
            //                     ->get();
            $dataPr = DB::select("
            SELECT a.id, a.trans_no, a.date_create, b.nama, a.status, a.pr_file, a.bukti_bayar, a.paid_date, a.total_price, a.reason FROM atk_storeq_header a
            ...
            SELECT COUNT(b.nama) as filtered FROM atk_storeq_header a
            INNER JOIN master_vendor b ON a.vendor_id = b.id WHERE a.deleted_at is NULL
            and a.reason like '%$search%'
            or 
            a.trans_no like '%$search%'
        ...
        if (!empty($dataPr)) {
            foreach ($dataPr as $ro) {
                $edit = url('atk/restock', $ro->id);
                $edit_paid = url('atk/restock_after_paid', $ro->id);
                $delete = $ro->id;
                ...
                $row['pr_file'] = "<a href='". asset('uploads/stock_atk/invoice/') ."/".$ro->pr_file."' target='_blank'>". $ro->pr_file ."</a> ";
                $row['paid_date'] = $ro->paid_date;
                $row['bukti_bayar'] = "<a href='". asset('uploads/stock_atk/bukti_bayar/') ."/".$ro->bukti_bayar."' target='_blank'>". $ro->bukti_bayar ."</a> ";
    ...
    public function tr_stock()
    {
        $data['emp'] = Employee::all();
        $data['dd'] = Employee::all();
        $data['dd_item'] = MasterStock::where('kategori','ATK')->get();
        $data['dd_gudang'] = MasterGudang::where('kategori','ATK')->get();
        // running number
        $code = "/TRO/ATK/";
        $bulan = Carbon::now()->format('m');
        $tahun = Carbon::now()->format('Y');
        ...
        // end of running number
        return view('atk.transfer.index',$data);
    }
    ...
    //    }
        ...
        if ($simpan_header) {
            $bal_asal = AtkDetails::where('id_barang',$request->id_barang)->where('gudang_id',$request->asal)->whereIn('status',['PAID','TRANSFERED','OUT'])->orderBy('id','desc')->first(['balance']);
            // asal
            $detail_asal = new AtkDetails;
            ...
            $detail_asal->save();
            // tujuan
            $bal_tujuan = AtkDetails::where('id_barang',$request->id_barang)->where('gudang_id',$request->tujuan)->whereIn('status',['PAID','TRANSFERED','OUT'])->orderBy('id','desc')->first(['balance']);
            $detail_tujuan = new AtkDetails;
    ...
    public function get_balance($id_barang, $id_gudang){
        $bal = AtkDetails::where('id_barang',$id_barang)->where('gudang_id',$id_gudang)->whereIn('status',['PAID','TRANSFERED','OUT'])->orderBy('id','desc')->first(['balance']);
        return $bal;
    }
    ...
    public function chained_gudang(Request $request)
    {
        if ($request->asal) {
            return $data = DB::table('master_gudang')
                ->where('id', '!=', $request->asal)->where('kategori','ATK')
                ->get(['id','nama as text']);
        }
        else {
            return $data = DB::table('master_gudang')
                ->where('id', '!=', $request->tujuan)->where('kategori','ATK')
                ->get(['id','nama as text']);
        }
    }
    ...
    public function TroHeaderList(Request $request)
    {
        ...
        // jika tidak ada get pada pencarian 
        if (empty($request->input('search.value'))) {
            $dataTransfer = AtkTroHeader::offset($start)
                                ->limit($limit)
                                ->orderBy($order, $dir)
                                ->get();
        } else {
    ...
    public function io_stock()
    {
        ...
        // end of running number
        return view('atk.out_stock.index', $data);
    }

    public function out_add(Request $request)
    {
        $header = new AtkOutHeader;
        $header->trans_no = $request->trans_no;
        $header->id_barang = $request->id_barang;
        $header->tanggal = $request->tanggal;
    ...
    public function outStockList(Request $request)
    {
        ...
        // jika tidak ada get pada pencarian 
        if (empty($request->input('search.value'))) {
            $dataOutStock = AtkOutHeader::offset($start)
                                ->limit($limit)
                                ->orderBy($order, $dir)
                                ->get();
        } else {
        ...
        if (!empty($dataOutStock)) {
            foreach ($dataOutStock as $ro) {
                $last_id = AtkDetails::where('id_barang', $ro->id_barang)->where('gudang_id',$ro->id_gudang)->orderBy('id', 'desc')->first()->id;
                $id_header = AtkDetails::where('id_barang', $ro->id_barang)->where('gudang_id',$ro->id_gudang)->where('id_header',  $ro->id)->first()->id;
                $delete = $ro->id;
                $row['no'] = $no;
                $row['trans_no'] = $ro->trans_no;
                $row['barang'] = $ro->master_barang->nama;
